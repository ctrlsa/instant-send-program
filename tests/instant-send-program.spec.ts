...
const programAddress = new PublicKey("4khKXMz3ttSaoxuwJ6nB93SB2PSjvj3FZP4E1gCPGHKW");
...
describe('Instant Transfer', () => {
    ...
    it('Initialize Transfer SOL', async () => {
        ...
        const context = await startAnchor("", [{name: "instant_send_program", programId: programAddress}], []);
        ...
        const expectedHashHex = "838e12ed6f077da89f4e3dcec5a9a8e8a21ed0714019176b8f95b935976acf50";
        ...
        if (computedHashHex !== expectedHashHex) {
            throw new Error(`Hash mismatch! Expected: ${expectedHashHex}, Got: ${computedHashHex}`);
        }
        
        const [escrowAccountPDA, escrowAccountBump] = await anchor.web3.PublicKey.findProgramAddressSync(
            [
                SEED_ESCROW_SOL,
        ...
        if (hashOfSecret.length !== 32) {
            throw new Error(`Hash must be exactly 32 bytes. Got ${hashOfSecret.length} bytes`);
        }
        ...
        .rpc();
        const escrowAccount = await transferProgram.account.escrowSolAccount.fetch(escrowAccountPDA);
